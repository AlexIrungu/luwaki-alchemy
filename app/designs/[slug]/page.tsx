...
/** Shapes Kent has delivered geometry for. The rest show as arriving. */
const DELIVERED: Shape[] = ["coffin"];
    ...
    <div className="relative overflow-hidden px-6 pb-32 pt-36">
      ...
      <section className="relative mx-auto mt-32 max-w-6xl">
        ...
        <ul className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-5">
          {SHAPES.map((shape) => {
            const ready = DELIVERED.includes(shape) && still;
            return (
              <li key={shape}>
                <div className="relative aspect-[3/4] overflow-hidden border border-line bg-panel">
                  {ready ? (
                    <Image src={still} alt={`${product.name}, ${shape}`} fill sizes="(min-width: 640px) 14rem, 45vw" className="scale-110 object-cover" />
                  ) : (
                    <span className="grid h-full place-items-center font-mono text-[9px] tracking-[0.25em] text-ink-faint">
                      ARRIVING
                    </span>
                  )}
                </div>
