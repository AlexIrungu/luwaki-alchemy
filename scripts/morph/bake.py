#!/usr/bin/env python3
"""
Bakes each design into height + colour maps for the home-page hero morph.

    python3 scripts/morph/bake.py

Reads Kent's raw exports in source/ (the same files `npm run models` uses) and
writes into public/morph/, per design:
  <slug>-height.png  R,G = top surface height (16-bit) · B = underside (8-bit)
  <slug>-color.png   RGB = linear base colour of the top surface
                     A = 255 pattern · 160 resin shell · 0 outside the nail
plus manifest.json describing the grid.

Every design is centred on its coffin shell, so the maps line up texel for
texel — that is what lets the hero blend one design's surface into the next.
A design without the full-length shared shell is skipped (Fractured Relic:
its cracked pattern is its own shell). Overhangs flatten to the highest
surface; at hero size that reads fine.
"""
import base64, colorsys, json, re, unicodedata
...
def bake(path, slug):
    ...
    if hi[2] - lo[2] < 30:
        return "no full-length shared shell"
    tris = tris - (lo + hi) / 2
    ...
    Image.fromarray(height, "RGB").save(OUT / f"{slug}-height.png", optimize=True)
    Image.fromarray(rgba, "RGBA").save(OUT / f"{slug}-color.png", optimize=True)
...
def main():
    ...
    print(f"\n{len(baked)} designs baked · grid {W}×{H} · {OUT / 'manifest.json'}")
